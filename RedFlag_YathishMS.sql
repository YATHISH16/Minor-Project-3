...
SELECT 
    ...
    t5.top5_volume, 
    m.total_volume, 
    (t5.top5_volume / m.total_volume) AS top5_ratio
FROM 
    top5_merchant_volume t5
JOIN 
    merchant_total_volume m ON t5.merchant_id = m.merchant_id
WHERE 
    (t5.top5_volume / m.total_volume) > 0.60
ORDER BY 
    top5_ratio DESC;
...
SELECT 
    user_id, 
    COUNT(*) AS just_under_threshold_count
FROM 
    transactions
WHERE 
    amount = 9999.00
...
HAVING 
    COUNT(*) >= 10
ORDER BY 
    just_under_threshold_count DESC;
...
gaps AS (
    SELECT 
        ...
        TIMESTAMPDIFF(DAY, prev_txn_time, txn_time) AS gap_days
    FROM 
        tx_with_lag
),
first_large_gap AS (
    SELECT 
        user_id, 
        MIN(gap_end_time) AS first_gap_end
    FROM 
        gaps
    WHERE 
        gap_days >= 90
...
SELECT 
    t.user_id, 
    COUNT(*) AS post_gap_count
FROM 
    transactions t
JOIN 
    first_large_gap f ON t.user_id = f.user_id
WHERE 
    t.txn_time >= f.first_gap_end
GROUP BY 
    t.user_id
HAVING 
    COUNT(*) >= 15
...
monthly_counts AS (
    ...
    FROM 
        user_months um
    LEFT JOIN 
        transactions t ON um.user_id = t.user_id 
...
user_metrics AS (
    SELECT 
        ...
        MAX(txn_count) AS peak_monthly_txn
    FROM 
        monthly_counts
    GROUP BY 
        user_id
...
SELECT 
    ...
    avg_monthly_txn, 
    peak_monthly_txn, 
    (peak_monthly_txn / avg_monthly_txn) AS spike_ratio
FROM 
    user_metrics
WHERE 
    peak_monthly_txn >= 20 
    AND (peak_monthly_txn / avg_monthly_txn) >= 5
ORDER BY 
    spike_ratio DESC;
